...
const APIS = [
  {
    ...
    replicate: "replicator:set_instance(e, cts.Target)",
    stop: "replicator:stop_instance(e, cts.Target)",
  },
  {
    key: "component",
    title: "component api",
    replicate: "world:add(e, Jecs.pair(replicator.components.instance, cts.Target))",
    stop: "world:remove(e, Jecs.pair(replicator.components.instance, cts.Target))",
  },
] as const;
...
for (const api of APIS) {
  describe(api.title, () => {
    test("reconciles an instance in a replicated container", async () => {
      const part = `${api.key}_replicated`;
      ...
      const result = await client.runCode({
        ...
        source: /* lua */ `
            ...
            for _, inst in world:query(cts.Target) do
                if typeof(inst) == "Instance" and inst.Name == "${part}" then
                    return true
                end
            end
            return false
    ...
    test("defers until the instance enters a replicated container", async () => {
      ...
      await server.runCode({
        target: "play:server",
        cacheRequires: true,
        source: /* lua */ `game:GetService("ServerStorage"):FindFirstChild("${part}").Parent = workspace`,
      })
